...
export const seedChatData: RequestHandler = async (req, res) => {
  try {
    ...
    // Create users
    const owner = await db.collection("users").findOne({ email: "owner@test.com" }) ||
      (await db.collection("users").insertOne({ name: "Owner", email: "owner@test.com", userType: "seller", createdAt: new Date() })).ops?.[0];

    const buyer = await db.collection("users").findOne({ email: "buyer@test.com" }) ||
      (await db.collection("users").insertOne({ name: "Buyer", email: "buyer@test.com", userType: "buyer", createdAt: new Date() })).ops?.[0];

    // Ensure we have proper docs
    const ownerId = owner?._id || (await db.collection("users").findOne({ email: "owner@test.com" }))._id;
    const buyerId = buyer?._id || (await db.collection("users").findOne({ email: "buyer@test.com" }))._id;

    // Create property
    const property = await db.collection("properties").findOne({ title: "Sample Seed Property" }) ||
    ...
    const propertyId = property?._id || (await db.collection("properties").findOne({ title: "Sample Seed Property" }))._id;

    // Create conversation
    let conversation = await db.collection("conversations").findOne({ property: propertyId, buyer: buyerId.toString(), seller: ownerId.toString() });
    if (!conversation) {
